...
import { useGetCommunityQuery, useListListingsQuery } from "@/apollograph/generated";
...
const CommunityContent = ({ params }: { params: any }) => {
  const communityId = parseInt(params.communitySlug, 10);
    const {data: communityData, loading: communityLoading, error:communityError } = useGetCommunityQuery({
      variables: {
        id: communityId
      }
    })
    ...
    const { data: listingData, loading:listingLoading, error: listingError } = useListListingsQuery({
      variables:{
        filters: {
  ...
  return (
    <section
      className={`lg:w-[73%] w-full relative lg:block block`}
    >
      ...
      <div className="flex justify-between items-start py-6 ">
        <div className="space-y-1 ">
          ...
          <div className="flex items-center  text-secondColor ">
            <span className=" text-base font-lexed ">{community?.memberCount} members</span>
          ...
          <div className="flex items-center text-secondColor ">
            ...
            <span className="text-base">{community?.location?.name ?? "Bangladesh"}</span>
          </div>
        </div>
