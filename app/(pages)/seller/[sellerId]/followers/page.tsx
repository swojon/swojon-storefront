  ...
  return (
    <section className="grid xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4">
      {followers && followers.map(follower => (
        <div key={follower.id} className="space-y-3 p-4 border border-gray-100 shadow-md bg-white rounded-md">
          ...
          <div className="flex items-center gap-2 ">
            ...
            <Link href={`/seller/${follower.user.id}`} className="border border-activeColor  text-activeColor   rounded-lg py-1 text-center  sm:text-xs text-xs hover:shadow-lg  cursor-pointer transition ease-in-out delay-150 duration-300 px-3">
              See Details
            </Link>
          </div>
        </div>
