...
function AppliedFilter() {
    ...
    return (
      <>        {filters.map((fi)  => (
        <div key={fi} className="bg-gray-100 px-3 py-1 flex gap-2.5 items-center rounded-2xl">
            <span className="text-sm">{fi} stars</span>{" "}
    ...
    {filters.length > 0 ? 
    <button onClick={() => handleRemoveAll()} className="bg-activeColor px-3 py-1 text-sm text-white rounded-2xl">
        Clear All
      </button>: null
    }
