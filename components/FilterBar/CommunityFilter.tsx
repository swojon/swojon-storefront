...
const CommunityFilter = () => {
  ...
  const handleChange = (val: any) => {
    ...
    else  applied = appliedCommunities.filter(item => item !== val.target.value)
    // conso setAppliedCommunities([...appliedCommunities, val.target.value])le.log(val.target.name)
    const params = new URLSearchParams(searchParams.toString())
    applied.length > 0 ? params.set("community", applied.join(',')) : params.delete('community')
  ...
  const filteredCommunities = !!query ? communities?.filter(com => com.name?.toLowerCase().includes(query.toLowerCase())) : communities; 
